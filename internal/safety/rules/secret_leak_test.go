...
func TestSecretLeakRule(t *testing.T) {
	...
	tests := []struct {
		...
		},
		{
			...
			message:     "The key is sk-ant-api03-AAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAA-AAAAAAAA",
			messageType: safety.MessageTypeAssistant,
			wantMatch:   true,
			wantDetail:  "Anthropic API key",
		},
		{
			name:        "OpenAI API key",
			message:     "Here is your key: sk-ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuv",
			messageType: safety.MessageTypeAssistant,
			wantMatch:   true,
			wantDetail:  "OpenAI API key",
		},
		{
			name:        "HuggingFace token",
			message:     "Token: hf_ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghij",
			messageType: safety.MessageTypeAssistant,
			wantMatch:   true,
			wantDetail:  "HuggingFace token",
		},
		{
			name:        "npm token",
			message:     "npm_ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghij",
			messageType: safety.MessageTypeAssistant,
			wantMatch:   true,
			wantDetail:  "npm token",
		},
		{
			name:        "DigitalOcean token",
			message:     "dop_v1_" + strings.Repeat("a", 64),
			messageType: safety.MessageTypeAssistant,
			wantMatch:   true,
			wantDetail:  "DigitalOcean token",
		},
		{
